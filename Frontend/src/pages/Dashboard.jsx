      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 border-2 border-emerald-200 dark:border-emerald-800/60 rounded-2xl p-5 space-y-3 shadow-sm hover:shadow-md transition-all">
            ...
            {isLoading
              ...
              : <div className="text-2xl font-extrabold text-slate-900 dark:text-white">
                  {totalSolved} <span className="text-xs text-slate-400 font-normal">/ 150</span>
                </div>
            }
          ...
          <div className="bg-white dark:bg-slate-900 border-2 border-amber-200 dark:border-amber-800/60 rounded-2xl p-5 space-y-3 shadow-sm hover:shadow-md transition-all">
            ...
            <p className={`text-xs font-semibold flex items-center gap-1 ${streak > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>
          ...
          {/* Solved Today */}
          <div className="bg-white dark:bg-slate-900 border-2 border-cyan-200 dark:border-cyan-800/60 rounded-2xl p-5 space-y-3 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Solved Today</span>
              <Calendar className="w-4 h-4 text-cyan-500" />
            </div>
            {isLoading
              ? <div className="h-8 rounded-lg bg-slate-200 dark:bg-slate-700 animate-pulse" />
              : <div className="text-2xl font-extrabold text-slate-900 dark:text-white">
                  {solvedToday} <span className="text-xs text-slate-400 font-normal">Problems</span>
                </div>
            }
            <p className={`text-xs font-semibold flex items-center gap-1 ${solvedToday > 0 ? 'text-cyan-600 dark:text-cyan-400' : 'text-slate-400 dark:text-slate-500'}`}>
          ...
          <div className="bg-white dark:bg-slate-900 border-2 border-amber-200 dark:border-amber-800/60 rounded-2xl p-5 space-y-3 shadow-sm hover:shadow-md transition-all">
            ...
            {isLoading
              ...
              : <div className="text-2xl font-extrabold text-slate-900 dark:text-white">
                  ...
                  <span className="text-xs text-slate-400 font-normal ml-1">Global</span>
                </div>
            }
                    ...
                    <Link key={p.id} to={`/patterns/${p.slug}`}>
                      <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                        <div className="flex items-center gap-2.5">
                          <span className="text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 code-font">
