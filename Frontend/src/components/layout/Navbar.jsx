          ...
          <div className="hidden md:flex items-center space-x-1">
            ...
            {isAuthenticated ? (
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-3 w-64 border border-white/10 rounded-2xl shadow-2xl py-2 z-50 bg-[#0a0a0f] backdrop-blur-xl ring-1 ring-black ring-opacity-5">
                    <div className="px-4 py-3 border-b border-white/10">
                      <div className="font-semibold text-white truncate">{getDisplayName()}</div>
                      <div className="text-xs text-gray-400 truncate">{user?.email}</div>
                    </div>
                    
                    <Link to="/profile" className="flex items-center px-4 py-2.5 text-sm text-gray-200 hover:text-white hover:bg-white/10 transition-colors" onClick={() => setShowUserMenu(false)}>
                      <User className="w-4 h-4 mr-2 text-primary" /> Profile
                    </Link>
                    
                    <Link to="/settings" className="flex items-center px-4 py-2.5 text-sm text-gray-200 hover:text-white hover:bg-white/10 transition-colors" onClick={() => setShowUserMenu(false)}>
                      <Settings className="w-4 h-4 mr-2 text-purple-400" /> Settings
                    </Link>
                    
                    <button onClick={handleLogout} className="flex items-center w-full px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 transition-colors">
              ...
              <div className="flex items-center space-x-3 ml-2">
                <Link to="/login"><Button variant="ghost" className="font-medium hover:bg-primary/10 rounded-xl px-5 text-sm">Login</Button></Link>
                <Link to="/register"><Button className="font-medium bg-gradient-to-r from-primary to-purple-600 hover:opacity-90 rounded-xl px-5 shadow-lg border-0 text-sm">Sign Up</Button></Link>
              </div>
            )}
          </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {isAuthenticated && (
              <div className="pt-4 mt-4 border-t border-white/10">
                ...
                <Link to="/profile" onClick={() => setIsOpen(false)} className="block px-3 py-2 text-base font-medium text-foreground hover:bg-white/5 rounded-md">Profile</Link>
